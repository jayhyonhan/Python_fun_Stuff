...
def convert_shape_format(shape):
    positions = []
    format_var = shape.shape[shape.rotation % len(shape.shape)]

    for i, line in enumerate(format_var):
        row = list(line)
    ...
    for i in range(len(grid)):
        ...
        for j in range(len(grid[i])):
            pygame.draw.line(surface, (128, 128, 128), (sx + j * block_size, sy),
                             (sx + j * block_size, sy + j * play_height))
...
def draw_window(surface, grid):
    ...
    label = font.render('Tetris', True, (0, 255, 255))
